    ...
    for (int it0 = 0; it0 < 10; it0++) {
      for (int mask = 1; mask < min(1<<target.size(), 1<<5); mask++) {
	      ...
	      for (int it = 0; it < maxiters; it++) {
...

	        {
	          ...
	          if (ok) {
	            ...
	            if (ok)
		            rets.emplace_back(pieces.dag, cp, cnt_pieces+1, sum_depth, max_depth);
	          }
	      ...
	      */
	      ...
	      rets.emplace_back(pieces.dag, ret, cnt_pieces, sum_depth, max_depth);
      }
    }
  }
...
vector<Candidate> evaluateCands(const vector<Candidate>&cands, vector<pair<Image,Image>> train) {
  ...
  for (const Candidate& cand : cands) {
    ...
    if (goods)
      ret.emplace_back(cand.pieces, imgs, score);
  }
