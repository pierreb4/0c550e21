struct Candidate {
  ...
  Candidate(const vector<DAG> &dag_, vImage_ imgs_, double score_) : imgs(imgs_), score(score_) {
    dag = dag_;
    cnt_pieces = sum_depth = max_depth = -1;
  }
  Candidate(const vector<DAG> &dag_, vImage_ imgs_, int cnt_pieces_, int sum_depth_, int max_depth_) :
